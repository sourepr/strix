...
STRIKE_API_BASE = "https://models.strix.ai/api/v1"


class Config:
    """Configuration Manager for Strike."""
    ...
    strike_reasoning_effort = "high"
    strike_llm_max_retries = "5"
    strike_memory_compressor_timeout = "30"
    llm_timeout = "300"
    _LLM_CANONICAL_NAMES = (
        ...
        "litellm_base_url",
        "ollama_api_base",
        "strike_reasoning_effort",
        "strike_llm_max_retries",
        "strike_memory_compressor_timeout",
        "llm_timeout",
    )

    # Tool & Feature Configuration
    perplexity_api_key = None
    strike_disable_browser = "false"

    # Runtime Configuration
    strike_image = "ghcr.io/usestrix/strix-sandbox:0.1.13"
    strike_runtime_backend = "docker"
    strike_sandbox_execution_timeout = "120"
    strike_sandbox_connect_timeout = "10"

    # Telemetry
    strike_telemetry = "1"
    strike_otel_telemetry = None
    strike_posthog_telemetry = None
...
def resolve_llm_config() -> tuple[str | None, str | None, str | None]:
    """Resolve LLM model, api_key, and api_base based on STRIKE_LLM prefix.

    Returns:
        tuple: (model_name, api_key, api_base)
        - model_name: Original model name (strike/ prefix preserved for display)
        - api_key: LLM API key
        - api_base: API base URL (auto-set to STRIKE_API_BASE for strike/ models)
    """
    ...
    if model.startswith("strike/"):
        api_base: str | None = STRIKE_API_BASE
    else:
        api_base = (
