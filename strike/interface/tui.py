...
from strike.agents.StrikeAgent import StrikeAgent
from strike.interface.streaming_parser import parse_streaming_content
from strike.interface.tool_components.agent_message_renderer import AgentMessageRenderer
from strike.interface.tool_components.registry import get_tool_renderer
from strike.interface.tool_components.user_message_renderer import UserMessageRenderer
from strike.interface.utils import build_tui_stats_text
...
def get_package_version() -> str:
    try:
        return pkg_version("strike-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: StrikeTUIApp | None = None

    def set_app_reference(self, app: "StrikeTUIApp") -> None:
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Strike Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit Strike?", id="quit_title"),
            Grid(
...
class StrikeTUIApp(App):  # type: ignore[misc]
    ...
    def _setup_cleanup_handlers(self) -> None:
        def cleanup_on_exit() -> None:
            from strike.runtime import cleanup_runtime
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            StrikeTUIApp._append_renderable(combined, item)
        return StrikeTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(StrikeTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                StrikeTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                StrikeTUIApp._append_renderable(combined, inner)
            else:
    ...
    def _start_scan_thread(self) -> None:
        def scan_target() -> None:
            try:
                ...
                try:
                    agent = StrikeAgent(self.agent_config)
    ...
    def _send_user_message(self, message: str) -> None:
        ...
        try:
            from strike.tools.agents_graph.agents_graph_actions import _agent_instances
        ...
        try:
            from strike.tools.agents_graph.agents_graph_actions import send_user_message_to_agent
    ...
    def action_confirm_stop_agent(self, agent_id: str) -> None:
        try:
            from strike.tools.agents_graph.agents_graph_actions import stop_agent
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in StrikeTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in StrikeTUIApp._ICON_PREFIXES:
...
async def run_tui(args: argparse.Namespace) -> None:
    """Run strike in interactive TUI mode with textual."""
    app = StrikeTUIApp(args)
    await app.run_async()
