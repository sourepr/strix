...
def _resolve_repo_diff_scope(
    ...
    if _is_repo_shallow(repo_path):
        raise ValueError(
            "Strike requires full git history for diff-scope. Please set fetch-depth: 0 "
            "in your CI config."
        )
...
def _is_http_git_repo(url: str) -> bool:
    ...
    try:
        req = Request(check_url, headers={"User-Agent": "git/strike"})  # noqa: S310
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "strike_repos" / run_name
