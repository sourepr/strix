...
if os.getenv("STRIKE_SANDBOX_MODE", "false").lower() == "false":
...
_SERVER_TIMEOUT = float(Config.get("strike_sandbox_execution_timeout") or "120")
SANDBOX_EXECUTION_TIMEOUT = _SERVER_TIMEOUT + 30
SANDBOX_CONNECT_TIMEOUT = float(Config.get("strike_sandbox_connect_timeout") or "10")
...
def _format_tool_result(tool_name: str, result: Any) -> tuple[str, list[dict[str, Any]]]:
    ...
    if result_str is None:
        ...
        if len(final_result_str) > 8000:
            start_part = final_result_str[:3000]
            end_part = final_result_str[-3000:]
