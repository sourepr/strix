...
from strike.telemetry.flags import is_otel_enabled
from strike.telemetry.utils import (
    TelemetrySanitizer,
    append_jsonl_record,
    bootstrap_otel,
    format_span_id,
...
class Tracer:
    ...
    def _setup_telemetry(self) -> None:
        ...
        (
            ...
            tracer_name="strike.telemetry.tracer",
        )
        ...
        if self._otel_tracer is not None:
            try:
                with self._otel_tracer.start_as_current_span(
                    ...
                    span.set_attribute("strike.event_type", event_type)
                    span.set_attribute("strike.source", source)
                    span.set_attribute("strike.run_id", self.run_id)
                    span.set_attribute("strike.run_name", self.run_name or "")

                    if status:
                        span.set_attribute("strike.status", status)
