...
def _process_dynamic_content(content: str) -> str:
    if "{{DYNAMIC_SKILLS_DESCRIPTION}}" in content:
        try:
            from strike.skills import generate_skills_description
...
def _get_schema_path(func: Callable[..., Any]) -> Path | None:
    ...
    return get_strike_resource_path("tools", folder, schema_file)


def _is_sandbox_mode() -> bool:
    return os.getenv("STRIKE_SANDBOX_MODE", "false").lower() == "true"


def _is_browser_disabled() -> bool:
    if os.getenv("STRIKE_DISABLE_BROWSER", "").lower() == "true":
        return True

    from strike.config import Config

    val: str = Config.load().get("env", {}).get("STRIKE_DISABLE_BROWSER", "")
