...
if not SANDBOX_MODE:
    raise RuntimeError("Tool server should only run in sandbox mode (STRIKE_SANDBOX_MODE=true)")

parser = argparse.ArgumentParser(description="Start Strike tool server")
...
async def _run_tool(agent_id: str, tool_name: str, kwargs: dict[str, Any]) -> Any:
    from strike.tools.argument_parser import convert_arguments
    from strike.tools.context import set_current_agent_id
    from strike.tools.registry import get_tool_by_name
