...
def resolve_strike_model(model_name: str | None) -> tuple[str | None, str | None]:
    """Resolve a strike/ model into names for API calls and capability lookups.

    Returns (api_model, canonical_model):
    - api_model: openai/<base> for API calls (Strike API is OpenAI-compatible)
    - canonical_model: actual provider model name for litellm capability lookups
    Non-strike models return the same name for both.
    """
    if not model_name or not model_name.startswith("strike/"):
        return model_name, model_name

    base_model = model_name[7:]
    api_model = f"openai/{base_model}"
    canonical_model = STRIKE_MODEL_MAP.get(base_model, api_model)
